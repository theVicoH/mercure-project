...
export class MessageUseCase implements IMessageUseCase {
  ...
  async createMessage(conversationId: number, senderId: number, message: string){
    ...
    }, process.env.MERCURE_PUBLISHER_JWT_KEY, expiration);
    await this.services.sse.publish("https://example.com/my-private-topic", {
