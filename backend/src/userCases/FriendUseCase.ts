...
export class FriendUseCase implements IFriendUseCase {
  ...
  async addFriend(userId: number, friendUsername: string) {
    const friendFound = await this.services.userService.findUser(friendUsername);
    if (!friendFound) {
      throw new Error("This user don' exist");
    }
    const friendId = friendFound.id;

    const exists = await this.services.friendService.checkFriendship(userId, friendId);
    if (exists) {
